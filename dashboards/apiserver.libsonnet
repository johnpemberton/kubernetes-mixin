  ...
  grafanaDashboards+:: {
    'apiserver.json':
      ...
      local workQueueAddRate =
        ...
        .addTarget(prometheus.target('sum(rate(workqueue_adds_total{%(kubeApiserverSelector)s, instance=~"$instance", %(clusterSelector)s}[5m])) by (instance, name)' % $._config, legendFormat='{{instance}} {{name}}'));
      ...
      local workQueueDepth =
        ...
        .addTarget(prometheus.target('sum(rate(workqueue_depth{%(kubeApiserverSelector)s, instance=~"$instance", %(clusterSelector)s}[5m])) by (instance, name)' % $._config, legendFormat='{{instance}} {{name}}'));
      ...
      local workQueueLatency =
        ...
        .addTarget(prometheus.target('histogram_quantile(0.99, sum(rate(workqueue_queue_duration_seconds_bucket{%(kubeApiserverSelector)s, instance=~"$instance", %(clusterSelector)s}[5m])) by (instance, name, le))' % $._config, legendFormat='{{instance}} {{name}}'));
      ...
      local etcdCacheEntryTotal =
        ...
        .addTarget(prometheus.target('etcd_helper_cache_entry_total{%(kubeApiserverSelector)s, instance=~"$instance", %(clusterSelector)s}' % $._config, legendFormat='{{instance}}'));
      ...
      local etcdCacheEntryRate =
        ...
        .addTarget(prometheus.target('sum(rate(etcd_helper_cache_hit_total{%(kubeApiserverSelector)s,instance=~"$instance", %(clusterSelector)s}[5m])) by (instance)' % $._config, legendFormat='{{instance}} hit'))
        .addTarget(prometheus.target('sum(rate(etcd_helper_cache_miss_total{%(kubeApiserverSelector)s,instance=~"$instance", %(clusterSelector)s}[5m])) by (instance)' % $._config, legendFormat='{{instance}} miss'));
      ...
      local etcdCacheLatency =
        ...
        .addTarget(prometheus.target('histogram_quantile(0.99,sum(rate(etcd_request_cache_get_duration_seconds_bucket{%(kubeApiserverSelector)s,instance=~"$instance", %(clusterSelector)s}[5m])) by (instance, le))' % $._config, legendFormat='{{instance}} get'))
        .addTarget(prometheus.target('histogram_quantile(0.99,sum(rate(etcd_request_cache_add_duration_seconds_bucket{%(kubeApiserverSelector)s,instance=~"$instance", %(clusterSelector)s}[5m])) by (instance, le))' % $._config, legendFormat='{{instance}} miss'));
      ...
      local memory =
        ...
        .addTarget(prometheus.target('process_resident_memory_bytes{%(kubeApiserverSelector)s,instance=~"$instance", %(clusterSelector)s}' % $._config, legendFormat='{{instance}}'));
      ...
      local cpu =
        ...
        .addTarget(prometheus.target('rate(process_cpu_seconds_total{%(kubeApiserverSelector)s,instance=~"$instance", %(clusterSelector)s}[5m])' % $._config, legendFormat='{{instance}}'));
      ...
      local goroutines =
        ...
        .addTarget(prometheus.target('go_goroutines{%(kubeApiserverSelector)s,instance=~"$instance", %(clusterSelector)s}' % $._config, legendFormat='{{instance}}'));
      ...
      .addTemplate(
        template.new(
          'instance',
          '$datasource',
          'label_values(apiserver_request_total{%(kubeApiserverSelector)s, %(clusterSelector)s}, instance)' % $._config,
          refresh='time',
          includeAll=true,
          sort=1,
        )
